...
/**
 * Bloc "Résumé rapide" du dashboard (5 compteurs). Dérivé en mémoire des
 * commandes déjà écoutées en temps réel par Dashboard.tsx (stats.statusCounts
 * + orders.length) — aucune requête Firestore supplémentaire.
 *
 * Pas de "vs hier" ici : cohérent avec le reste du dashboard, qui n'a pas
 * encore d'historique de snapshots pour calculer une évolution (voir le
 * commentaire équivalent au-dessus de `stats` dans Dashboard.tsx).
 *
 * Affiché uniquement sur desktop (le parent l'entoure de `hidden md:block`) ;
 * sur mobile, voir QuickSummaryFab.tsx qui réutilise la même liste ITEMS.
 */
